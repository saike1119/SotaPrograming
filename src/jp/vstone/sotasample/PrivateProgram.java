...
public class PrivateProgram {
	...
	static CRobotMem mem = new CRobotMem();
	// Sota用モーション制御クラス
	static CSotaMotion motion = new CSotaMotion(mem);

	static CRoboCamera cam = new CRoboCamera("/dev/video0", motion);
	...
	public static void main(String args[]) {
		...
		while (true) { // Thread.sleepで間隔をあけて繰り返し（代わりに顔認識や踊りなどを入れてもよい）
			...
			// service.setUsernameAndPassword("88088328-6c86-448d-86e7-f620d43e8c8a",
			// "omEsuNDPVPRK");//資格情報のユーザ名とパスワードに<>も除いて置き換える
			...
			RecognizeOptions opts = new RecognizeOptions.Builder().contentType(HttpMediaType.AUDIO_WAV) // .wavファイルの場合はAUDIO_WAV
					.continuous(true) //
					.interimResults(true) //
					.model(model) //
					.build();
			...
			for (int i = 0; i < t_list.size(); i++) {
				...
				for (int j = 0; j < t_str_list.length; j++) {
					if (t_str_list[j].indexOf("D_") != 0) { // 先頭文字がD_（フィラーとして認識）以外のとき
						// System.out.println(t_str_list[j] +
						// t_str_list[j].indexOf("D_"));
			...
			if (subject_str.length() > 0) {
				...
				JsonArray bindings = json_obj.get("results").getAsJsonObject().get("bindings").getAsJsonArray();

				String abst_str = "";
				if (bindings.size() > 0) {
					abst_str = bindings.get(0).getAsJsonObject().get("object").getAsJsonObject().get("value")
							.getAsString();
				}
				...
				if (abst_str.length() > 0) { // DBPediaからの結果が得られた時のセリフの生成
					...
					robot_speech = subject_str + "をWikipediaで調べてみたら、「";
					...
					robot_speech += "をWikipediaで調べたけど、わからなかったよ";
				}
				// ここまで：DBPediaの検索結果に特化した結果の処理～セリフの生成
			} else { // 探すべき単語が聞き取れなかったときのセリフ生成
						// robot_speech
						// ="うまく聞き取れなくて，すみません．もう一度，尋ねますので，しばらくお待ちください．";
	 ...
	 *
	 */
	public static boolean getWavFileByCallPOST(String strPostUrl, String strContentType, String formParam,
			String target) {
